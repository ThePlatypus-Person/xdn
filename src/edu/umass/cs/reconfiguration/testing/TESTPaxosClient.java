 ...
 * @author V. Arun
 * <p>
 *         FIXME: Near-duplication of TESTPaxosClient. Remove redundant code by
 *         extending from TESTPaxosClient.
 */
public class TESTPaxosClient {
    ...
    private static Logger log = Logger
            .getLogger(TESTPaxosClient.class.getName());
    ...
    private class ClientPacketDemultiplexer extends
        ...
        protected Object processHeader(byte[] message, NIOHeader header) {
            if (ByteBuffer.wrap(message).getInt() == AppRequest.PacketType.META_REQUEST_PACKET
                    .getInt()) {
                try {
                    return MetaRequestPacket.getMetaRequestPacket(message);
                } catch (UnsupportedEncodingException | UnknownHostException
                         | JSONException e) {
                    e.printStackTrace();
                }
            }
    ...
    protected TESTPaxosClient(int id, NodeConfig<Integer> nc)
        ...
        this.timer = new Timer(TESTPaxosClient.class.getSimpleName()
                + myID);
    ...
    private RequestPacket makeRequest() {
        ...
        RequestPacket req = new MetaRequestPacket(reqID,
    ...
    protected boolean makeAndSendRequest(String paxosID) throws JSONException,
        ...
        return TESTPaxosClient.this
                .sendRequest(TESTPaxosClient.this
                        .makeRequest(paxosID));
    }
    ...
    protected boolean makeAndSendRequestCallable(String paxosID)
        ...
        executor.submit(new Callable<Boolean>() {
            public Boolean call() {
                // long t = System.nanoTime();
                RequestPacket req = TESTPaxosClient.this
                        .makeRequest(paxosID);
                try {
    ...
    private static void initResponseTracker(
            TESTPaxosClient[] clients, int numRequests) {
    ...
    protected static void sendTestRequests(int numReqs,
                                           TESTPaxosClient[] clients, double load)
            throws JSONException, IOException, InterruptedException,
            ExecutionException {
    ...
    private static double getTotalThroughput(
            TESTPaxosClient[] clients, long startTime) {
        int totalExecd = 0;
        for (int i = 0; i < clients.length; i++) {
     ...
     */
    private static double probeCapacity(double load,
                                        TESTPaxosClient[] clients) throws JSONException,
            IOException, InterruptedException, ExecutionException {
    ...
    protected static void twoPhaseTest(int numReqs,
                                       TESTPaxosClient[] clients) throws InterruptedException,
            JSONException, IOException, ExecutionException {
        // begin first run
        long t1 = System.currentTimeMillis();
