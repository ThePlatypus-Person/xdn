...
public class MessagingTask {
    ...
    public static MessagingTask getNonLoopback(MessagingTask mtask, int myID) {
        ...
        if (arrayContains(mtask.recipients, myID)) {
            if (mtask.recipients.length > 1)
                return new MessagingTask(Util.filter(mtask.recipients, myID),
                        mtask.msgs);
            else
                return null;
        }
     ...
     */
    public static MessagingTask[] combine(MessagingTask[] array1,
                                          MessagingTask[] array2) {
        int length1 = (array1 != null ? array1.length : 0);
        int length2 = (array2 != null ? array2.length : 0);
        MessagingTask[] mtasks = new MessagingTask[length1
                + length2];
        for (int i = 0; i < length1; i++)
            mtasks[i] = array1[i];
        for (int i = 0; i < length2; i++)
            mtasks[length1 + i] = array2[i];
        return mtasks;
    }
     ...
     */
    public static MessagingTask[] combine(MessagingTask mtask,
                                          MessagingTask[] array) {
        if (mtask == null && array == null)
            return null;
        else if (mtask != null)
