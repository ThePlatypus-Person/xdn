...
import edu.umass.cs.xdn.bluegreenprimarybackup.interfaces.BlueGreenBackupableApplication;
...
import edu.umass.cs.xdn.service.*;
 ...
 *
 * <p>It replaces XdnGigapaxosApp with a clean separation between deterministic and
 * non-deterministic services. It owns: - a serviceRegistry mapping serviceName → ServiceType - a
 * DeterministicService instance handling all deterministic services - a NonDeterministicService
 * instance handling all non-deterministic services - a shared HTTP request cache (shared between
 * both service types)
 *
 * <p>GigaPaxos sees exactly one APPLICATION class and calls execute(), checkpoint(), and restore()
 * on this class. XdnApp routes each call to the correct service based on the registry, which is
...
public class XdnApp
  ...
  public XdnApp(String[] args) {
    ...
    logger.log(
        Level.INFO,
        "{0}:XdnApp initializing with args: {1}",
  ...
  public Request getRequest(String stringified)
      throws edu.umass.cs.reconfiguration.reconfigurationutils.RequestParseException {
    edu.umass.cs.xdn.request.XdnRequestType packetType =
        edu.umass.cs.xdn.request.XdnRequest.getQuickPacketTypeFromEncodedPacket(stringified);
    if (packetType == null) {
      throw new edu.umass.cs.reconfiguration.reconfigurationutils.RequestParseException(
          new Exception("Unknown XDN packet: " + stringified));
    }
    ...
    if (packetType.equals(XdnRequestType.XDN_SERVICE_HTTP_REQUEST)) {
      ...
      if (cachedId != null) {
        ...
        if (cached instanceof XdnHttpRequest xdnReq) {
          if (xdnReq.getHttpResponse() == null && XdnHttpRequest.doesHasResponse(stringified)) {
    ...
    if (packetType.equals(XdnRequestType.XDN_HTTP_REQUEST_BATCH)) {
      ...
      XdnHttpRequestBatch batch =
          XdnHttpRequestBatch.createFromBytes(
    ...
    throw new edu.umass.cs.reconfiguration.reconfigurationutils.RequestParseException(
        new Exception("Unknown XDN packet type: " + packetType));
  }
  ...
  public void putInitialState(String name, int epoch, String state) {
    throw new RuntimeException(
        "XdnApp.putInitialState is unimplemented " + "for name=" + name + " epoch=" + epoch);
  }
  ...
  public void validateInitialState(String initialState) throws InvalidInitialStateException {
    if (!initialState.startsWith(ServiceProperty.XDN_INITIAL_STATE_PREFIX)) {
      throw new InvalidInitialStateException(
          "Invalid initial state prefix, expecting " + ServiceProperty.XDN_INITIAL_STATE_PREFIX);
    }
    String encoded = initialState.substring(ServiceProperty.XDN_INITIAL_STATE_PREFIX.length());
    ServiceProperty property;
    try {
      property = ServiceProperty.createFromJsonString(encoded);
    } catch (Exception e) {
      throw new InvalidInitialStateException("Invalid ServiceProperty JSON: " + e.getMessage());
    }
  ...
  public boolean waitUntilReady(String serviceName, AbstractStateDiffRecorder.LiveDirType backup) {
  ...
  private boolean waitUntilInstanceReady(
      String serviceName, ServiceInstance instance, Set<String> containerNames) {
    ...
    for (int i = 0; i < instance.containerNames.size(); i++) {
      ...
      if (component.getHealthEndpointPath() != null) {
        ...
        ready =
            sandboxManager.waitUntilHttpReady(
  ...
  /**
   * Caches a deserialized request so getRequest() can return it without re-deserializing when
   * GigaPaxos calls it on the same node.
   */
  ...
  public boolean stopBackupContainer(
  ...
  /**
   * Determines service type from the state string on the first restore() call. Falls back to
   * NON_DETERMINISTIC if parsing fails — primary-backup is always safe, just slower.
   */
  private ServiceType detectServiceType(String state) {
    try {
      ...
      logger.log(
          Level.WARNING,
          "{0}:XdnApp detectServiceType() failed: {1}; " + "defaulting to NON_DETERMINISTIC",
  ...
  /**
   * Parses a ServiceProperty from any known state string format: xdn:init:<servicePropertyJSON>
   * xdn:final:<epoch>::<servicePropertyJSON>::<state> non-deter:load:xdn:init:<servicePropertyJSON>
   */
  ...
  /**
   * Configures request matchers on a deserialized XdnHttpRequest based on the service's
   * ServiceProperty. Falls back gracefully if the service is not yet registered.
   */
