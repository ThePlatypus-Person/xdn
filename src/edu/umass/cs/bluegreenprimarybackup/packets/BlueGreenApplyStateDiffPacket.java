 ...
 *
 * <p>Corresponds to "ApplyStateDiff{stateDiff, stateDiffCount, myID/primaryID,
 * currPlacementEpoch/primaryEpoch, currPlacement/placement}" in the event/action protocol
 * pseudocode.
 *
 * <p>The stateDiff field may either be raw diff bytes (small diffs) or a LargeCheckpointer handle
 * string encoded as bytes (large diffs, e.g. the 200MB MySQL bootstrap case) -- the distinction is
 * not yet encoded as an explicit field here; see TODO below.
 *
 * <p>TODO: per the earlier design discussion, large diffs (e.g. >500KB bootstrap diffs) should
 * route through LargeCheckpointer.createCheckpointHandle() instead of embedding raw bytes inline.
 * Consider adding an explicit boolean isCheckpointHandle field rather than relying on runtime
 * sniffing via LargeCheckpointer.isCheckpointHandle() on the stateDiff bytes.
 *
 * <p>TODO: the placementEpoch field below is checked against currPlacementEpoch in the
 * Notify(ApplyStateDiff) handler, but currPlacementEpoch is also the field name used by
 * Notify(StartEpoch) for a DIFFERENT purpose (tracking the epoch of the current placement's
 * primary). Confirm this is the intended field to compare against -- flagged during design
 * discussion, not yet resolved.
 *
 * <p>Wire format (plain Java, no protobuf): [4 bytes packetType][8 bytes packetId][4 bytes
 * serviceName length] [serviceName bytes][4 bytes placement][4 bytes primaryEpoch] [4 bytes
...
public class BlueGreenApplyStateDiffPacket extends BlueGreenPrimaryBackupPacket
  ...
  public int hashCode() {
    int result =
        Objects.hash(packetId, serviceName, placement, primaryEpoch, primaryID, stateDiffCount);
